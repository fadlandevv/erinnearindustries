...
export default async function CustomJerseyPage() {
  const [pricingItems, opts] = await Promise.all([getPricingItems(), getCustomProductOptions('jersey')])
  ...
  const sizeOptions   = opts.sizes.length  > 0 ? opts.sizes.map(s => s.label) : (DEFAULT_SIZES['jersey'] ?? [])
